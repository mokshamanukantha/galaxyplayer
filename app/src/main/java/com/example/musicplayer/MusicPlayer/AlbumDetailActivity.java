 ...
 */
public class AlbumDetailActivity extends Activity implements AdapterView.OnItemClickListener, View.OnClickListener, View.OnLongClickListener, SeekBar.OnSeekBarChangeListener, AdapterView.OnItemLongClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        album_id = intent.getLongExtra(TAG_ID, TAG_NUM);
        albumSongList = new ArrayList<Song>();

        songlist = Utilities.getSongListForAlbum(this, album_id);
        for(int i=0;i<songlist.length;i++)
        {
            for(int j=0;j<songList.size();j++)
            {
                ...
                {
                    albumSongList.add(songList.get(j));
                }
            }
        }
        songAdt = new SongAdapter(this, albumSongList);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        mPlayer.setSong(mPlayer.getSongArrayPos(albumSongList.get(Integer.parseInt(view.getTag().toString())).getID()));
        mPlayer.playSong();
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.btnBackward) {
            ...
            if (currentSongIndex > 0) {
                mPlayer.setSong(mPlayer.getSongArrayPos(albumSongList.get(currentSongIndex - 1).getID()));
                mPlayer.playSong();
                currentSongIndex = currentSongIndex - 1;
            } else {
                mPlayer.setSong(mPlayer.getSongArrayPos(albumSongList.get(albumSongList.size() - 1).getID()));
                mPlayer.playSong();
                currentSongIndex = albumSongList.size() - 1;
            }
        }
        if (v.getId() == R.id.btnForward) {
            sTitle.setText("" + mPlayer.getSongTitle());
            currentSongIndex = mPlayer.getCurrentSongIndex();
            if (currentSongIndex < (albumSongList.size() - 1)) {
                mPlayer.setSong(mPlayer.getSongArrayPos(albumSongList.get(currentSongIndex + 1).getID()));
                mPlayer.playSong();
                currentSongIndex = currentSongIndex + 1;
            } else {
                mPlayer.setSong(mPlayer.getSongArrayPos(albumSongList.get(0).getID()));
                mPlayer.playSong();
                currentSongIndex = 0;
            }
        }
    ...
    {
        imageView.setImageBitmap(albumSongList.get(mPlayer.getCurrentSongIndex()).getImage());
        sTitle.setText("" + albumSongList.get(mPlayer.getCurrentSongIndex()).getTitle());
    }
